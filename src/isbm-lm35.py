...
def cleanup_gpio():
    GPIO.output(settings.fan_right_pin, GPIO.LOW)  # Sağ motor kapat
    GPIO.output(settings.fan_left_pin, GPIO.LOW)  # Sol motor kapat
    GPIO.output(16, GPIO.LOW)  # Sağ motor kapat
    GPIO.output(23, GPIO.LOW)  # Sol motor kapat
    GPIO.output(24, GPIO.LOW)  # Sol motor kapat
    ...
    def run(self):
        ...
        while self.counter < self.desired_success_count:
            ...
            for i, chan in enumerate(channels):
             olcum.append(read_temp_from_channel(chan))
            ...
            if len(olcum) < 15:
                olcum.extend([404.0] * (15 - len(olcum)))
        ...
        for i, temp in enumerate(temps[:-1]):  # Son parametre counter, onu atlıyoruz
            ...
            if (temp == "404.0" or temp == "x") and rowPosition > 0:
                ...
                try:
                    ...
                    item = QTableWidgetItem(str(temp_value))
                    tempsdb.append(str(temp_value))
                except ValueError:
